...
#' @param .network_variables Variables to set for the nextwork.
#' @param .filter_variable Which variables to filter.
#' @param .filter_value The filter value.
#' @param .adjustment_variable Variables to adjust for.
#'
...
nc_make_network <- function(.dataset, .alpha, .network_variables) {
  ...
  est.pcor.skel.DAG.adj <- function(.dat, .alpha_val, .network_variables) {
    ...
    adj_matrix <- igraph::get.adjacency(igraph::igraph.from.graphNEL(skel_est@graph))

...
net.coupler.in<-function(graph_skel,dat,dat_compl,exp_dat,DE){
  ...
  for(i in node_names){                                          #net.coupler.in loop over all metabolite nodes
    ...
    if(is.na(DE)==FALSE){adjset_names<-setdiff(adjset_names,as.vector(DE))
    ...
    glmulti_obj<-glmulti(y="met_out_data",                                      #response variable to be predicted: selected metabolite
                         ...
                         intercept=FALSE,                                       #no intercept included in candidate models
...
getExp.coef.peroutcome<-function(object,outcome,exposure){
  ...
  for(i in seq(along=nbm)){
    ...
    mm_coef_temp2<-data.frame(Model=as.character(object[[outcome]]$Model_summaries[[i]]$Model),
                              Nbmds=Nbmds,
...
mult.stat<-function(sum_netin,MinMod,adjust_method,rule_1 = 11,rule_1_cut = 0.1,rule_2 = 5,rule_2_cut = 0.05,rule_3 = 4,rule_4 = 3, ass_rule1=9, round_number){
  ...
  p_adj<-sapply(p_adjust_M,function(meth){p.adjust(sum_netin_sum$marg_P,meth)})            #calculate adjusted p-values for marg_P
...
amb.met.loop.CC<-function(exp_dat,graph_skel,dat,dat_compl,DE,exposure,met_map,adjust_method,round_number,adjM_norename,met_group){
  ...
  for(i in 1:CC_numb_max){
    ...
    if(length(amb_met_loop[[i]]$con_comp)==0){break}
    if(length(amb_met_loop[[i]]$con_comp[[1]]$all_CC)<i){break}

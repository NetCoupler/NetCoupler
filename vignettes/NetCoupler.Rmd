...
nc_plot_external_var(standardized_data,
                     metabolite_network,
                     outcome_direct_effects)
...
model_edges <- outcome_direct_effects %>% 
    mutate(to = as.numeric(as.factor(index_node)),
           from = length(unique(outcome)) + length(unique(index_node)),
           direct_effect = na_if(direct_effect, "none"),
           estimate = if_else(is.na(direct_effect), NA_real_, estimate)) %>% 
    select(from, to, estimate, p_value, direct_effect)

network_edges <- graph_data_prep %>% 
    activate("edges") %>% 
    as_tibble()

all_edges <- bind_rows(model_edges, network_edges)

models_graph <- tbl_graph(
    nodes = tibble(
        name = graph_data_prep %>%
            activate("nodes") %>%
            pull(name) %>%
            union(unique(outcome_direct_effects$outcome))
    ),
    edges = all_edges
) %>% 
    activate("edges") %>% 
    mutate(estimate = if_else(direct_effect == "none", NA_real_,
                              estimate),
           weight = if_else(is.na(weight), estimate, weight),
           direct_effect = if_else(is.na(direct_effect), 
...
layout <- models_graph %>% 
    create_layout("gem") %>% 
    mutate(y = if_else(name == unique(outcome_direct_effects$outcome),
                       mean(y), y),
           x = if_else(name == unique(outcome_direct_effects$outcome),
                       max(x) * 2.25, x)) 

models_graph %>%
    ggraph("manual", x = layout$x, y = layout$y) +
